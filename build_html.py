...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* 简答题长文本输入：多行 textarea，自动换行不横向滚动 */
  .fill-textarea {
  ...
  /* 锁定后显示的用户答案（长文本换行） */
  .short-my-answer {
    margin-top: 10px; font-size: 14.5px; line-height: 1.8;
    color: var(--ink-2);
    background: rgba(255,255,255,0.45);
    padding: 12px 14px; border-radius: var(--r-row);
...
// 迁移旧版 qid（含 index 后缀）→ 新版 qid（卷-题型-题号）
function migrateState() {
  const lookup = {};
  QUESTIONS.forEach(q => { lookup[q.paper + "-" + q.type + "-" + q.no] = true; });
  let changed = false;
  // answered
  const newAnswered = {};
  Object.keys(state.answered).forEach(k => {
    const base = k.replace(/-\d+$/, "");
    if (lookup[base] && !newAnswered[base]) { newAnswered[base] = state.answered[k]; changed = true; }
    else if (lookup[k]) { newAnswered[k] = state.answered[k]; }
  });
  if (changed) state.answered = newAnswered;
  // 旧版简答记录（无 pending 字段）→ 标记为已自评（保持锁定语义）
  Object.keys(state.answered).forEach(k => {
    const a = state.answered[k];
    if (a && a.my === "self-ok" || a && a.my === "self-no") {
      if (a.text === undefined) { a.text = a.my; a.pending = false; }
    }
  });
  // wrongIds / favIds
  state.wrongIds = state.wrongIds.map(k => k.replace(/-\d+$/, "")).filter(k => lookup[k]);
  state.favIds = state.favIds.map(k => k.replace(/-\d+$/, "")).filter(k => lookup[k]);
  // 去重
  state.wrongIds = [...new Set(state.wrongIds)];
  state.favIds = [...new Set(state.favIds)];
}
function saveState() {
  if (!storageOk) return;
  try { localStorage.setItem(LS_KEY, JSON.stringify(state)); } catch(e) {}
}
// 题目唯一标识：卷+题型+题号（不含 index——同一题在任何题序下 qid 一致，防止重复计分）
function qidOf(q) { return q.paper + "-" + q.type + "-" + q.no; }
...
function filterQuestions() {
  ...
  if (state.mode === "wrong") {
    ...
    list = list.filter(x => w.has(qidOf(x.q)));
  }
  if (state.mode === "fav") {
    const f = new Set(state.favIds);
    list = list.filter(x => f.has(qidOf(x.q)));
  }
...
function render(animate) {
  ...
  function buildBody() {
    ...
    if (q.type === "choice") {
      ...
      q.options.forEach((opt) => {
        ...
        // 已答锁定：onclick 置空，视觉加 locked（不透明度降低）
        const click = answered ? "" : `onclick="answer('${qid}','${letter ? letter[1] : ""}')"`;
        h += `<div class="opt ${cls}${answered ? ' locked' : ''}" ${click}>${escapeHtml(opt)}</div>`;
      });
      h += '</div>';
      ...
      ["√","×"].forEach(v => {
        ...
        const click = answered ? "" : `onclick="answer('${qid}','${v}')"`;
        h += `<div class="judge-btn ${cls}${answered ? ' locked' : ''}" ${click}>${v}</div>`;
      });
      h += '</div>';
    } else if (q.type === "fill") {
      const locked = answered ? " disabled" : "";
      h += `<input class="fill-input" id="fill-input" type="text" placeholder="请输入答案" value="${answered && answered.my ? escapeHtml(String(answered.my)) : ""}"${locked}>`;
      h += `<button class="fill-submit" onclick="submitFill('${qid}')"${locked}>${answered ? "已提交" : "提交答案"}</button>`;
    } else {
      // 简答题三态：未提交（textarea）→ 待自评（我的答案+参考答案+自评）→ 已自评（锁定）
      h += '<div class="short-ans">';
      if (!answered) {
        // 未提交：textarea + 提交
        h += `<textarea class="fill-textarea" id="short-input" placeholder="请输入你的答案"></textarea>`;
        h += `<button class="fill-submit" onclick="submitShort('${qid}')">提交答案</button>`;
      } else if (answered.pending) {
        // 待自评：我的答案 + 参考答案 + 自评按钮
        h += '<div class="short-my-answer">' + escapeHtml(String(answered.my)) + '</div>';
        h += '<button class="ans-toggle" onclick="toggleAns(this)"><span class="chev">▼</span>查看参考答案</button><div class="ans-wrap"><div><div class="ans-body">' + escapeHtml(q.answer) + '</div></div></div>';
        h += '<div class="self-row">';
        h += `<button class="self-btn self-no" onclick="answer('${qid}','self-no')">❌ 我没答对</button>`;
        h += `<button class="self-btn self-ok" onclick="answer('${qid}','self-ok')">✅ 我答对了</button>`;
        h += '</div>';
      } else {
        // 已自评：锁定（我的答案 + 参考答案 + 反馈）
        h += '<div class="short-my-answer">' + escapeHtml(String(answered.text || answered.my)) + '</div>';
        h += '<button class="ans-toggle" onclick="toggleAns(this)"><span class="chev">▼</span>查看参考答案</button><div class="ans-wrap"><div><div class="ans-body">' + escapeHtml(q.answer) + '</div></div></div>';
        h += `<div class="feedback show ${answered.ok ? "ok" : "err"}">${answered.ok ? "✅ 自评：答对了！" : "❌ 自评：没答对（已加入错题本）"}</div>`;
      }
      h += '</div>';
    }
  ...
  const doRender = () => {
    ...
    html += '</div></div>';
    area.innerHTML = html;
    // 输入框回车 = 提交答案（填空/简答），永不跳题
    if (q.type === "fill" && !answered) {
      const inp = document.getElementById("fill-input");
      if (inp) {
        inp.focus();
        inp.addEventListener("keydown", e => {
          if (e.key === "Enter") { e.preventDefault(); submitFill(qid); }
        });
      }
    } else if (q.type === "short" && !answered) {
      const ta = document.getElementById("short-input");
      if (ta) {
        ta.addEventListener("keydown", e => {
          // Enter 提交；Shift+Enter 换行
          if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); submitShort(qid); }
        });
      }
    }
...
// ===== 答题（提交即锁定；错题模式=重做不计统计；简答三态：未提交→待自评→已自评） =====
function answer(qid, val) {
  const i = state.order[state.idx];
  const q = QUESTIONS[i];
  if (q.type === "short") {
    // 简答：自评（仅从"已提交待自评"状态进入）
    const prev = state.answered[qid];
    if (!prev || !prev.pending) return;
    const ok = val === "self-ok";
    record(qid, ok, val, prev.my);
  } else {
    // 选择/判断：非错题模式已答锁定；错题模式允许重做
    if (state.answered[qid] && state.mode !== "wrong") return;
...
/**
 * 提交记录：
 * - 正常作答（首次）：计数 totalAnswered，答对 totalOk+1
 * - 错题模式重做：不计入统计，只维护错题集（答对移除/答错保留）
 * - 答对：若该题在错题集（跨轮次遗留），移除
 */
function record(qid, ok, my, text) {
  ...
  // 错题集维护（跨轮次）
  if (ok) {
    state.wrongIds = state.wrongIds.filter(x => x !== qid);
  } else {
...
document.getElementById("btn-random").onclick = () => {
  ...
  if (state.shuffle) {
    ...
    const list = filterQuestions().filter(i => !state.answered[qidOf(QUESTIONS[i])]);
...
document.addEventListener("keydown", e => {
  // 回车永远不触发下一题（输入框内回车=提交答案，由输入框自身的 keydown 处理）
