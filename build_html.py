...
<style>
  ...
  :root {
    ...
    --spring-standard: cubic-bezier(0.34, 1.2, 0.64, 1);   /* 弹出：轻微过冲 */
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .drop-panel {
    ...
    transition: opacity 0.22s var(--spring-settle), transform 0.32s var(--spring-standard), visibility 0s 0.28s;
  }
  .drop-panel.show {
    opacity: 1; visibility: visible;
    transform: none;
    transition: opacity 0.22s var(--spring-settle), transform 0.32s var(--spring-standard), visibility 0s;
  }
  .drop-opt {
  ...
  .card {
    ...
    padding: 22px 20px;
    position: relative;
    animation: glassIn 0.45s var(--spring-standard) both;
  }
  /* 切题时卡片淡入上浮 */
  .card.enter { animation: cardIn 0.45s var(--spring-standard) both; }
  @keyframes cardIn {
...
function render(animate) {
  ...
  let html = '<div class="card glass-frost' + (animate ? ' enter' : '') + '">';
  html += `<div class="q-head"><span class="crumb">卷<b>${q.paper}</b> · ${typeName} · 第 ${q.no} 题</span>`;
  html += `<button class="fav-btn ${isFav ? 'on' : ''}" onclick="toggleFav('${qid}')" title="${isFav ? '取消收藏' : '收藏本题'}">${isFav ? '★' : '☆'}</button>`;
  html += `<span class="tag" style="margin-left:auto;">${state.idx+1}/${state.order.length}</span></div>`;
  ...
  if (q.type === "choice") {
    ...
    html += `<button class="fill-submit" onclick="submitFill('${qid}')">${answered ? "重新提交" : "提交答案"}</button>`;
  } else {
    ...
    html += `<button class="self-btn self-ok ${answered && answered.my === 'self-ok' ? 'chosen' : ''}" onclick="answer('${qid}','self-ok')">✅ 我答对了</button>`;
    html += `<button class="self-btn self-no ${answered && answered.my === 'self-no' ? 'chosen' : ''}" onclick="answer('${qid}','self-no')">❌ 我没答对</button>`;
    html += '</div>';
  }
...
// ===== 导航（切题带卡片动画） =====
function next() {
  if (state.idx < state.order.length - 1) { state.idx++; saveState(); render(true); }
}
function prev() {
  if (state.idx > 0) { state.idx--; saveState(); render(true); }
}
...
document.getElementById("btn-random").onclick = () => {
  ...
  state.order = list; state.idx = 0; saveState(); render(true);
};
document.getElementById("btn-wrong").onclick = () => {
  ...
  state.idx = 0; state.order = filterQuestions(); saveState(); render(true);
};
...
function resetProgress() {
  ...
  state = { mode:"paper", paper:"ALL", type:"ALL", order:[], idx:0, answered:{}, wrongIds:[], favIds:[], totalAnswered:0, totalOk:0 };
  state.order = filterQuestions();
  saveState(); render(true);
}
